...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{1\linewidth}
    \centering
    \includegraphics[scale=0.5]{../fyp/assets/task-pics/reach-no-obs/random-scene-hierarchy.png}
...
conditions are not always ideal, so wanted to do some quick checks to get concrete results on how the agent behaves when the target is placed on the fringes of its field of vision. So created the thee tasks shown in Figure \ref{fig:no-obs-3-views}.
...
\missingfigure{add the task images from `ReachObs_Random/IndRandom'}
...
Another task which is likely to suffer from lack of viewpoints is a grasping task. Although, in a more nuanced way compared to the reaching with obstacles. \todo{Grasp_Simple and OTher}
