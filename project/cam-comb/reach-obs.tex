...
\subsection{Attending on a Camera in Given Combination}\todo{references for all the sections here?}
...
Given a set of RGB views I want to score them and weight them according to their perceived \emph{utility}, the first metric that comes to mind is colour. 
For every view, $\mathcal{V}$ and every pixel $\left( u, ~v\right)$ we calculate its euclidean distance to the target's colour: \( ||\mathcal{V}\left(u, ~v\right) - t_{colour}||\).

\subsection{Scoring the  Weighted Features}\todo{rewrite fixup}
; in our batch, $B$, we calculate a scalar \emph{score} where \(\mathcal{V}_{i, x} \in \mathbb{R}^{B \times {v}}, ~{v} \in \{ {RGB}_{wrist}, ~{RGB}_{left\_shoulder}, ~{RGB}_{right\_shoulder} \}\) the scores are then normalised using softmax: \( \tilde{\mathcal{V}}_{i, x} = \frac{\mathcal{V}_{i, x}}{\sum_{}^{k \in v} \mathcal{V}_{i, k} + 10^{-6}} \) to be a weighing; scalar term $10^{-6}$ being a\todo{a what, what is the correct term?} , indicating which view is the most important at the moment. 
